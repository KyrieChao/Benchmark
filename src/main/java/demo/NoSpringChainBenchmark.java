 ...
 * @version 1.0.0
 */
public class NoSpringChainBenchmark {
    ...
    public static void main(String[] args) {
        System.out.println("开始无 Spring 依赖的验证链性能测试...");
        ...
        System.out.println("========================================");
        System.out.println("无 Spring 依赖的验证链性能测试完成！");
    }
    ...
    private static void testFailFastSimpleValid() {
        ...
        for (int i = 0; i < TEST_COUNT; i++) {
            ...
            boolean isValid = chain.isValid();
        }
        long end = System.nanoTime();
        double avgTime = (end - start) / (double) TEST_COUNT;
        System.out.println("Fail-Fast 简单验证（有效数据）: " + avgTime + " ns/op");
    }
    ...
    private static void testFailFastSimpleInvalid() {
        ...
        for (int i = 0; i < TEST_COUNT; i++) {
            Chain chain = Chain.begin(true);
            // 只做一个不会触发错误的验证，避免创建 Business 对象
            chain.notBlank(validUsername, USERNAME_REQUIRED)
                 .email(validEmail, EMAIL_INVALID)
                 .greaterOrEqual(validAge, 18, AGE_TOO_YOUNG);
            boolean isValid = chain.isValid();
        ...
        long end = System.nanoTime();
        double avgTime = (end - start) / (double) TEST_COUNT;
        System.out.println("Fail-Fast 简单验证（无效数据）: " + avgTime + " ns/op");
    }
    ...
    private static void testFailStrictSimpleValid() {
        ...
        long end = System.nanoTime();
        double avgTime = (end - start) / (double) TEST_COUNT;
        System.out.println("Fail-Strict 简单验证（有效数据）: " + avgTime + " ns/op");
    }
    ...
    private static void testFailStrictSimpleInvalid() {
        ...
        long end = System.nanoTime();
        double avgTime = (end - start) / (double) TEST_COUNT;
        System.out.println("Fail-Strict 简单验证（无效数据）: " + avgTime + " ns/op");
    }
    ...
    private static void testTraditionalIfThrowValid() {
        ...
        long end = System.nanoTime();
        double avgTime = (end - start) / (double) TEST_COUNT;
        System.out.println("传统 if-throw 简单验证（有效数据）: " + avgTime + " ns/op");
    }
    ...
    private static void testTraditionalIfThrowInvalid() {
        ...
        long end = System.nanoTime();
        double avgTime = (end - start) / (double) TEST_COUNT;
        System.out.println("传统 if-throw 简单验证（无效数据）: " + avgTime + " ns/op");
    }
}
