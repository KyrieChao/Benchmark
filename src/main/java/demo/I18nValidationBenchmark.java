...
public class I18nValidationBenchmark {
    ...
    public static void main(String[] args) {
        ...
        // 测试 2: 普通消息验证（无效数据） - 暂时注释，避免 Spring 依赖问题
        // testNormalMessageInvalid();

        // 测试 3: 国际化消息验证（有效数据）
        testI18nMessageValid();

        // 测试 4: 国际化消息验证（无效数据） - 暂时注释，避免 Spring 依赖问题
    ...
    private static void testI18nMessageValid() {
        ...
        for (int i = 0; i < TEST_COUNT; i++) {
            try {
                Failure.begin()
                        .notBlank(validUsername, ResponseCode.of(40001, "Username is required"))
                        .email(validEmail, ResponseCode.of(40002, "Email is invalid"))
                        .fail();
            } catch (Exception e) {
                // 忽略异常
            }
    ...
    private static void testI18nMessageInvalid() {
        ...
        for (int i = 0; i < TEST_COUNT; i++) {
            try {
                Failure.begin()
                        .notBlank(invalidUsername, ResponseCode.of(40001, "Username is required"))
                        .email(invalidEmail, ResponseCode.of(40002, "Email is invalid"))
                        .fail();
            } catch (Exception e) {
                // 忽略异常
            }
